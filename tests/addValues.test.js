...
test('number + number = number', () => {
    expect(addValues(100, 55)).toBe(155);
});

test('array + bool = array', () => {
    expect(addValues([5,6], true)).toStrictEqual([5, 6, true]);
});

test('string + bool = string', () => {
    expect(addValues("543.987", true)).toBe("543.987true");
});

test('string + number = string', () => {
    expect(addValues("10.2", 55)).toBe("10.255");
});

test('bool + number = bool', () => {
    expect(addValues(false, 55)).toBe(false);
});
...
test('Symbol + number = string', () => {
    expect(addValues(Symbol(100), 55)).toBe("10055");
});

test('bigint + number = bigint', () => {
    expect(addValues(BigInt(100), 55)).toBe(155n);
});

test('bigint + symbol = string', () => {
    expect(addValues(BigInt(100), Symbol(55))).toBe("10055");
});